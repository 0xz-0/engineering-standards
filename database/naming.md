...
| `_minor` | 金额（该货币最小货币单位，用于非固定两位小数的场景——含多币种及单一非两位小数币种，需配合 `currency_code` + `currency_exponent` 换算，见[表设计规范 2.2](table-design.md#22-多币种例外commerce-multi-currency-exceptionmay)） | 大整数 | `amount_minor` |
